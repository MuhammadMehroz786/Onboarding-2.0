...
async function testTechLaunchLogin() {
  ...
  if (user) {
    ...
    if (user.client) {
      ...
      console.log('\n🎪 Social Platforms:');
      const socialPlatforms = JSON.parse(user.client.socialPlatforms || '[]');
      console.log('  Platforms:', socialPlatforms.join(', '));

      console.log('\n🏆 Competitors:');
      const competitors = JSON.parse(user.client.competitors || '[]');
      console.log('  Main Competitors:', competitors.join(', '));
    } else {
